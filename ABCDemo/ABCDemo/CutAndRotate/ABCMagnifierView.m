...
@interface ABCMagnifierView ()

@property (nonatomic, strong) UILabel *addLabel;

@end

@implementation ABCMagnifierView

- (void)dealloc {
    NSLog(@"ABCMagnifierView dealloc");
}
...
- (void)setMagnifierWidth:(float)magnifierWidth {
    ...
    self.frame = CGRectMake(0, 0, _magnifierWidth, _magnifierWidth);
    if (_magStyle == ABCMagnifierStyleCircular) {
...
- (void)setMagStyle:(ABCMagnifierStyle)magStyle {
    _magStyle = magStyle;
    switch (_magStyle) {
        case ABCMagnifierStyleCircular: {
            self.layer.cornerRadius = self.magnifierWidth / 2;
            self.layer.masksToBounds = YES;
        } break;
        case ABCMagnifierStyleSquare: {
